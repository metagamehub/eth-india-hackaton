	...
	return (
		<>
			...
			<div className="flex flex-col space-y-3 w-90 text-white">
				<h2 className="text-xl truncate ">LATEST ACTIVITY</h2>
				{!activities ? (
					...
					<>
							...
							activities.map((activity) => (
								<div className="flex h-10 bg-grey rounded-[15px] space-x-0 justify-between">
									<div className="gradientbox2 font-fire text-md py-2 px-2 mx-[3px] my-[3px] text-center sm:scale-85 lg:scale-90 xl:scale-100 rounded-[15px]">
										<h2 className="text-[15px]">+{activity.points_earned}</h2>
									</div>
									<p className="font-fire text-md py-2 truncate">{activity.metadata.eventType}</p>
						...
						{(!activities || activities?.length === 0) && (
							<>
								<div className="flex h-10 bg-grey rounded-[15px] space-x-0 justify-between">
									<p className="font-fire text-md py-2 pl-4 truncate">Your activity is empty</p>
								</div>
							</>
						)}
					</>
