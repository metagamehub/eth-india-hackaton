...
module.exports = {
    ...
    read: async function (req, res) {
        try {
            ...
            let item
            if (event_id)
                item = await Events.findOne({ where: { event_id: event_id } })
    ...
    delete: async function (req, res) {
        try {
            ...
            if (event_id)
                await Events.destroy({ where: { event_id: event_id } })
            else if (walletAddress)
    ...
    claimBadges: async function (req, res) {
        try {
            ...
            for (let eventType of eventTypes) {
                ...
                if (event && balances[index] == 0)
                    mintBadges(walletAddress, index)
                index++
            }
